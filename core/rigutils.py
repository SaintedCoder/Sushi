...
def create_rig(target="default"):
    ...
    arm_thing = bpy.data.armatures.new("Sushi")
    armObj = bpy.data.objects.new("Sushi", arm_thing)
...
def convert_rig_to_class(armature=None, rig_class=None, brand="Sushi"):
    ...
    if 1 == 0:
        for mod_name in bpy.context.preferences.addons.keys():
            ...
            if name == "Sushi":
                ...
                break
        armObj["Sushi"] = utils.get_Sushi_version()
    armObj["Sushi"] = "Post 2.8.2 - see rigs : build_sl_rig to re-enable"
...
def make_complete(
    ...
    if len(armObj.data.bones) != len(newObj.data.bones):
        ...
        print(
            "If you are attempting to import and repair a dae file use the Sushi dae importer instead."
        )
        return False
